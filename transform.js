...
const usage = "Usage: " + process.argv[0] + " " + process.argv[1] + " <JEDIT_EXPORT_DIR> <RESULT>";
...
function transformAndSave(objs) {
    ...
    var res = {
        "defaults": {
            "method": "POST",
            "path": "/buckets/default/collections/tickets/records",
        }
    };
...
function put(i, label, collection, tickets) {
    if (tickets.length > 0) {
        var batch = {
            "defaults": {
                "method": "POST",
                "path": "/buckets/default/collections/"+collection+"/records",
            }
        };
        ...
        return new Promise(function(resolve, reject) {
            ...
            var req = {
                method: "POST",
                uri: "http://localhost:8888/v1/batch",
                auth: {
                    user: "token",
                    pass: "tr0ub4d@ur"
                },
